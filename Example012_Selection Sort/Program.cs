/* Задача. Дан массив {1,5,4,3,2,6,7,1,1}. Упорядочить массив с 
   минимального до максимального значений элементов.*/
...

{
    ...
    for (int i = 0; i < count; i++)/* Пробегаем по всем элементам массива.*/
    {
        Console.Write($"{array[i]} ");/* Каждый элемент массива будет 
                                         выводиться на экран через пробел.*/
    }
                                  ...
                                  некоторый массив array.*/
{
    ...
    for (int i = 0; i < array.Length - 1; i++)/* Если мы начинаем поиск максимального от i+1 (ниже),
    ...
    {
        int minPosition = i;/* Определяем позицию элемента, для которого, в 
