...
диапазоне от -2 147 483 648 до 2 147 483 647.*/
    ...
    /* Итеративное решение.*/
    double FactorialFor(int n) /*Заданим функцию, которая будет принимать число (аргумент), факториал которого
    ...
    {
        double result = 1; /* ВВодим результирующую переменную и присваиваем ей значение (1), 
                              потому, что факториал считается от единицы.*/
        for (int i = 1; i <= n; i++) result *= i; /* Цикл. До тех пор, пока (i)- счётчик не достигнет 
        значения (n), то (i), при каждой итерации будем увеличивать на единицу, результат, каждый раз 
        умножать на текущее значение (i) и ...*/
        return result; /* после этого возвращать результат.*/
    }
...

    {
        ...
        else return n * FactorialRec(n - 1);  /* Иначе нужно текущее значение умножить на факториал 
        предыдущего значения числа. Обязательно вернуть значение (return).*/
    }
    ...
    /* Дальнейший код предназначен для того, чтобы вывести на экран результаты 
       подсчёта факториала каждого промежуточного числа от (1) до (n). Применим итеративный метод.*/
    ...
    будем вычислять факториал каждого числа (увеличивая (i), при каждой итерации, на (1).*/
    {
        if (FactorialRec(i) > 1.7 * Math.Pow((10), 308))//Если факториал числа более 1.7*10 в степени 308, то...
        {
        ...
        Console.WriteLine($"Факториал {i!} = {FactorialRec(i)}"); /* В ответ выведтся значение каждого
